...
test_that("Search is correct", {
    ...
    expect_true(all(unlist(lapply(res$matches$ingredients, function(x) "asparagus" %in% x))))
    ...
    expect_false(any(unlist(lapply(res$matches$ingredients, function(x) "asparagus" %in% x))))
    expect_error(search_recipes("bacon", allowed_ingredients = "asparagus2"))
    expect_warning(search_recipes("bacon", allowed_ingredients = "fried"))
})
